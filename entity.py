...
class MousePicker(object):
    ...
    def compute_mouse_ray(self, x, y, width, height):
        ...
        view_coords_point = convert_to_eye_coords(
            clip_coords_point, self._camera.get_projection_matrix()
        )
        world_coords_point = convert_to_world_coords(
            view_coords_point, self._camera.get_view_matrix()
        )
        ray = np.array(
            [world_coords_point[0], world_coords_point[1], world_coords_point[2]]
        )
