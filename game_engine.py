...
class GameEngine(QObject):
    ...
    def on_mouse_move(self, x, y):
        ...
        plane_point = find_plane_point(
            self._camera.eye, self._camera.eye + self._mouse_picker.ray * 500.0
        )
    ...
    def on_clicked(self, button, x, y):
        ...
        if (
            button == 0
            and self._board_table[self._curr_row][self._curr_col].status
            == TILE_OCCUPIED
        ):
            if not self._has_selected:
                ...
                self._board_table[self._selected_tile[0]][
                    self._selected_tile[1]
                ].status = TILE_OCCUPIED
                self._board_table[self._curr_row][self._curr_col].status = TILE_SELECTED
                self._board_table.selected = [self._curr_row, self._curr_col]
            ...
            self._has_selected
            and self._board_table[self._curr_row][self._curr_col].status == TILE_EMPTY
        ):
            ...
            if (
                self._board_table[self._curr_row][self._curr_col].status
                == TILE_SELECTED
            ):
